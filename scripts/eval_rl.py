...
Supports both legacy PPO and AlphaPoker agents.
"""
import numpy as np
from collections import deque
from pokerStats.rl import PokerEnv, NUM_ACTIONS
from pokerStats.rl.ppo_agent import PPOAgent, RolloutBuffer
from pokerStats.rl.self_play_trainer import ScriptedAgent, MultiAgentTrainer, CFG
...
def benchmark(agent, style, n_hands=2000, num_players=6):
    ...
    for _ in range(n_hands):
        ...
        while not done:
            ...
            mask = env.legal_mask()
            if pidx == 0:
                action, rf, _, _, _ = agent.get_action(o, mask, deterministic=True)
            else:
                action, rf, _, _, _ = opp.get_action(o, mask)
            obs, rews, done, _ = env.step(action, rf)
...
# ── Quick train (multi-agent) ──────────────────────────────────────────
def quick_train(n_hands=5000, num_players=6, use_alpha=True):
    if use_alpha:
        print(f"Training AlphaPoker agent for {n_hands} hands "
              f"(multi-agent, {num_players} players)...")
        cfg = dict(CFG)
        cfg["phase1_hands"] = n_hands
        cfg["phase2_hands"] = 0
        cfg["phase3_hands"] = 0
        cfg["log_freq"] = 500
        cfg["eval_freq"] = n_hands + 1  # skip eval
        cfg["save_freq"] = n_hands + 1  # skip saves
        cfg["render_every"] = n_hands + 1
        ...
        for h in range(n_hands):
            ...
            while not done:
                ...
                o = obs[pidx]
                mask = env.legal_mask()
                action, raise_frac, log_prob, value, entropy = agent.get_action(o, mask)
                obs_next, rewards, done, _ = env.step(action, raise_frac)

                if pidx == 0:
                    reward = rewards.get(0, 0.0)
                    buf.add(o, action, raise_frac, log_prob, reward, value,
                            float(done), mask)
                    if buf.full:
...
if __name__ == "__main__":
    ...
    parser.add_argument("--skip-train", action="store_true",
                        help="Benchmark a random agent (no training)")
    parser.add_argument("--legacy", action="store_true",
                        help="Use legacy PPO instead of AlphaPoker")
