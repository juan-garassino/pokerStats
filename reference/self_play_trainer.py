...
Training curriculum:
  Phase 1 — Pure self-play (agent vs copies of itself)
             → learns basic strategy, stops making huge blunders
  Phase 2 — League play (agent vs pool of frozen past checkpoints)
             → prevents strategy collapse, forces diverse opponent modeling
  Phase 3 — Fine-tune vs human-mimic opponents (scripted archetypes)
             → adapts to real human tendencies before live deployment

ELO tracking:
  Every 500 hands, run a 1000-hand evaluation tournament.
  Update ELO for all agents. Only promote to league if ELO improves.

Key insight from Pluribus (FB AI, 2019):
  Self-play alone collapses to a narrow strategy.
  The league of frozen opponents is what forces robustness.
"""
...
from ascii_renderer import render_training_stats
...
CFG = {
    ...
    "big_blind":        2.0,
    "render_every":     5000,    # render ASCII table every N hands
    ...
    # Self-play
    "self_play_update_freq":  500,    # hands between self-play opponent updates
    "league_size":            8,      # number of frozen opponents in pool
    "league_add_freq":        2000,   # hands between adding new member to league
    "eval_freq":              1000,   # hands between ELO evaluations
    "eval_hands":             500,    # hands per ELO evaluation
    "min_elo_gain":           10.0,   # minimum ELO gain to add to league

    # Training phases
    "phase1_hands":     50_000,   # pure self-play
    "phase2_hands":     200_000,  # league play
    "phase3_hands":     50_000,   # fine-tune vs archetypes
...
class ScriptedAgent:
    ...
    def get_action(self, obs: np.ndarray, legal_mask: np.ndarray,
        ...
        if self.style == "nit":
            ...
            # Raise preflop strong hands, fold marginal
            if street_idx == 0:
                if call_high:
...
class League:
    def __init__(self, max_size: int = 8):
        ...
        self.members:  list = []   # (agent, elo, metadata)
        self.elo_hist: list = []
    ...
    def add(self, agent: PPOAgent, tag: str = ""):
        ...
        if len(self.members) > self.max_size:
            # Remove weakest member (lowest ELO), keep diversity
    ...
    def sample(self) -> PPOAgent:
        """Sample a random opponent from the league (uniform)."""
        if not self.members:
            return None
        return random.choice(self.members)["agent"]

    def sample_weighted(self) -> PPOAgent:
        """Sample with probability proportional to ELO (favor stronger opponents)."""
        if not self.members:
            return None
...
class SelfPlayTrainer:
    ...
    def _get_opponents(self, phase: int) -> list:
        """
        Build the list of opponent agents for this hand.
        Phase 1: all copies of current agent (pure self-play)
        Phase 2: mix of current agent + league members
        Phase 3: mix of current + league + scripted archetypes
        """
    ...
    def _run_hand(self, phase: int) -> dict:
        """Run one complete hand. Returns stats."""
        obs      = self.env.reset()
    ...
    def _eval_elo(self, n_hands: int = 500) -> float:
        """
        Play eval hands vs league members and update ELO.
        Returns updated ELO.
        """
    ...
    def train(self):
        ...
        print(f"  Total hands: {self.cfg['phase1_hands']+self.cfg['phase2_hands']+self.cfg['phase3_hands']:,}")
        print(f"{'═'*70}\n")
        ...
        for phase, n_hands, phase_name in phase_schedule:
            ...
            for h in range(n_hands):
                ...
                # ASCII render occasionally
                if self.hand_num % self.cfg["render_every"] == 0:
                    self.env.render_mode = "ascii"
...
if __name__ == "__main__":
    ...
    parser.add_argument("--render", action="store_true", help="Show ASCII table every hand")
