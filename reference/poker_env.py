...
Design choices:
  • Observation:  147-dim vector (hand + board + betting history + position)
...
This is the hardest piece because:
  1. Variable number of active players per street
  2. Side pots (all-in scenarios)
  3. Correct blinds, position rotation across hands
  4. Handling all edge cases: split pots, dead money, etc.
"""
...
from ascii_renderer import render_table, render_hand
...
class PokerEnv:
    ...
    def __init__(
        ...
        big_blind:       float = 2.0,
        render_mode:     str   = "none",   # "ascii" | "none"
        max_raises:      int   = 4,        # max raises per street (prevents infinite loops)
    ):
    ...
    def _render_ascii(self) -> str:
        from ascii_renderer import render_table
        return render_table({
