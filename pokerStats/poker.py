...
class Poker(object):
    ...
    def isFour(
        ...
        Currank = sortedHand[
            1
        ].rank  # since it has 4 identical ranks,the 2nd one in the sorted listmust be the identical rank
        count = 0
        total_point = h * 13**5 + self.point(sortedHand)
        for card in sortedHand:
    ...
    def isFull(
        ...
        rank1 = sortedHand[
            0
        ].rank  # The 1st rank and the last rank should be different in a sorted list
    ...
    def isThree(self, hand):
        ...
        Currank = sortedHand[
            2
        ].rank  # In a sorted rank, the middle one should have 3 counts if flag=True
        mylist = []
        for card in sortedHand:
            mylist.append(card.rank)
    ...
    def isTwo(
        ...
        rank1 = sortedHand[
            1
        ].rank  # in a five cards sorted group, if isTwo(), the 2nd and 4th card should have another identical rank
    ...
    def isOne(
        ...
        if (
            mycount.count(2) == 2 and mycount.count(1) == 3
